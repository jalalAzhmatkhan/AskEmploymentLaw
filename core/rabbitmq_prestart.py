...
def init_rabbitmq_connection() -> bool:
    ...
    try:
        if is_rabbitmq_need_auth:
            logger.info(f"init_rabbitmq_connection: [ReqId: {str(uuid5())}] Connecting to RabbitMQ using Credentials.")
            ...
            logger.info(f"init_rabbitmq_connection: [ReqId: {str(uuid5())}] Connecting to RabbitMQ without using Credentials.")
            connection = pika.BlockingConnection(pika.ConnectionParameters(rabbitmq_host, rabbitmq_port, heartbeat=60))
        channel = connection.channel()

        logger.info(f"init_rabbitmq_connection: [ReqId: {str(uuid5())}] RabbitMQ Connection is Online.")
        response = True
        connection.close()
    except pika.exceptions.AMQPConnectionError as e:
        logger.error(f"init_rabbitmq_connection: [ReqId: {str(uuid5())}] Error connecting to RabbitMQ: {e}", exc_info=True)
